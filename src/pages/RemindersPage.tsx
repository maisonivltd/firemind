      ...
      <div className="space-y-4 animate-[fade-in_0.5s_ease-out]">
        ...
        {showAdd && (
          <div className="rounded-2xl border border-border bg-card p-4 space-y-4 animate-[slide-up_0.3s_ease-out]">
            <Input placeholder="Scrivi il tuo messaggio motivazionale..." value={newText} onChange={(e) => setNewText(e.target.value)} className="h-12 rounded-xl bg-background text-base" />
            <div className="space-y-2">
              <label className="text-sm font-semibold text-foreground flex items-center gap-1">
                <Clock className="h-4 w-4 text-primary" /> Orari notifica
              </label>
              {newTimes.map((time, i) => (
                <Input key={i} type="time" value={time} onChange={(e) => updateTime(i, e.target.value)} className="h-10 rounded-xl bg-background" />
              ))}
              {newTimes.length < 3 && (
                <button onClick={addTimeSlot} className="text-xs font-semibold text-primary hover:underline">+ Aggiungi orario (max 3)</button>
              )}
            </div>
            <div className="flex gap-2">
              <Button onClick={addReminder} className="warm-gradient-bg flex-1 rounded-xl text-primary-foreground font-bold hover:opacity-90">Salva</Button>
        ...
        <div className="space-y-3">
          {reminders.map((r) => (
            <div key={r.id} className={`rounded-2xl border border-border bg-card p-4 space-y-3 transition-opacity ${!r.active ? "opacity-50" : ""}`}>
              ...
              <div className="flex items-center justify-between">
                <div className="flex gap-2">
                  {r.times.map((t, i) => (
                    <span key={i} className="warm-gradient-bg rounded-lg px-2 py-0.5 text-xs font-bold text-primary-foreground">{t}</span>
                  ))}
                </div>
